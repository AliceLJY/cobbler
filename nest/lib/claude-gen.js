...
// 条子只点出该审哪个前提,不替她把判断写好(2026-09-17 Alice 拍板)。
// 缘由:09-04~09-16 的 13 张知识扭蛋卡里,7 张的「反面」条直接写好了反对意见,
// 12 张用「我怀疑」「我当时」这类她的口吻写模型自己的推测;书堆扭蛋 16 张里也有 8 张
// 带第一人称推测(均为关键词粗筛)。她整段复制去问大模型时,对方会把这些当成她本人的判断
// 顺着接;她自己读时也分不清哪个疑问是自己想到的。猜测不禁,但只能放在该条末尾、标明是 Cobbler 猜的。
// 两条管线共用这一份,避免措辞漂移;接条子那一头的说明见 tg-send.js 的 COBBLER_GUESS_NOTE。
export const FOLLOWUP_JUDGMENT_RULES = [
  '  别替她把答案写好:问题里只点出要审的是哪个前提、哪个断言、哪一步推理,',
  '  不要写出现成的反对意见、现成的反例、现成的漏洞或你猜的结论——那是她拿去问、自己想的部分。',
  '  你若有自己的猜测,只能放在该条最后,写成「(Cobbler 猜:……)」,问题主体里不许出现;猜测也只写短短一句。',
  '  口吻:条子是她复制出去问的,提问本身可以用她的第一人称(「请帮我…」「我这页记的是…」);',
  '  但推测、怀疑、猜测一律不许写成她的口吻(不写「我怀疑」「我猜」「我想到的是」「我当时大概」),',
  '  要么放进「(Cobbler 猜:……)」,要么删掉。',
];

// 条子写短,只问让人停下来想的问题(2026-09-19 Alice:「现在觉得写得太长,我都不想看,
// 我觉得提出让人思考的问题就行」)。
// 这是对 08-27 那两条的反转,不是忘了它们:当时两条 50 字的问题被硬截断在半句,
// 她要「这种分量」的问题,于是改成 5-7 条不限字数、每条把「审哪个前提 / 为什么承重 /
// 要什么证据」三层写透。三周实测修复后的卡:知识扭蛋每张约 7 条、条子中位 1,725 字,
// 书堆扭蛋 7 条、中位 2,504 字——分量全压在字数上,读的人先被字数劝退。
// 现在的判断:分量来自问得准,不来自写得长;三层推理留给回答的那一方去展开。
// 仍然只在 prompt 里要求写短,代码不截断——08-27 的硬截断就是把问题砍在了半句。
// 两条管线共用这一份,避免措辞漂移。
export const FOLLOWUP_BREVITY_RULES = [
  '  条子是她先读一眼、再整段复制去问的——读着费劲她就不想看了。',
  '  每条只问一件事,一句话问完,最好不超过 60 字,读完让人想停下来想一想。',
  '  直接问:不用先铺一句背景,也不用每条都拿「请帮我…」开头;抓手(名字、数字、判断)放进问句里。',
  '  写完数一下:超过 60 字就删掉铺垫,只留问句本身。',
  '  分量来自问得准,不来自写得长:背景、推理步骤、要拿什么证据才能坐实,',
  '  都留给回答的那一方去展开,不要写进问题里。宁可少问一条,也别把一条写成一段。',
];

// 口吻漂移的只记录检查(2026-09-19 加,影子期,只写日志不拦截)。
// FOLLOWUP_JUDGMENT_RULES 只活在 prompt 里,漏了就看不见:09-19 那张卡的第 7 问
// 「这个定性大概影响了我对它借鉴价值的判断」把模型的推测写成了她的话,而那一页的三条
// 拒因里根本没有「零代码」。三个模式是拿本机真卡校准过的(nest/data 被 gitignore,
// 卡本身不进公开仓):08-28~09-16 修复前 37 张卡报出 21 张;09-17~09-19 修复后 6 张
// 卡只报出上面那 1 条,零误报。裸关键词「大概」在修复后 3 张卡里命中 3 条,真违规只有
// 1 条(另两条是「每段大概要多少步骤」这种正常提问),所以不用裸关键词。
// 已知漏报形态:只写「我当时」不带含糊词的(可能是合法的「我当时记的是…」,不报)。
// 已知误报形态:「其他我可能接触过的东西」这种问题范围里的「我可能」。
// 条末「(Cobbler 猜:…)」里允许有推测,先剥掉再查。
const VOICE_LEAK_PATTERNS = [
  /我(怀疑|猜|估计|想到的是|觉得|担心|推测|直觉)/,
  /我(当时|那时|之前|原来|本来)?(大概|可能|也许|多半|恐怕)/,
  /(这个|那个|这条|这些|这种|这一)[^，。？?！!]{0,20}(大概|可能|也许|多半|恐怕)[^，。？?！!]{0,12}(影响|让|导致|误导|左右|决定)了?我/,
];
export function describeVoiceLeaks(followups) {
  if (!Array.isArray(followups)) return [];
  const leaks = [];
  followups.forEach((f, i) => {
    if (typeof f !== 'string') return;
    const main = f.split(/[（(]Cobbler 猜/)[0];
    for (const p of VOICE_LEAK_PATTERNS) {
      const m = main.match(p);
      if (m) { leaks.push(`第 ${i + 1} 条「…${main.slice(Math.max(0, m.index - 12), m.index + m[0].length + 8)}…」`); break; }
    }
  });
  return leaks;
}

// 写短的只记录检查(同为影子期):prompt 要的是约 60 字,这里按 90 字报,
// 给措辞留余量——只想知道模型是不是又滑回了一条写一段。
export const OVERLONG_FOLLOWUP_CHARS = 90;
export function describeOverlong(followups, limit = OVERLONG_FOLLOWUP_CHARS) {
  if (!Array.isArray(followups)) return [];
  return followups
    .map((f, i) => (typeof f === 'string' && f.trim().length > limit ? `第 ${i + 1} 条 ${f.trim().length} 字` : null))
    .filter(Boolean);
}
