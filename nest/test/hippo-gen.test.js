...
test('成功路径:含 followups 数组 → 返回卡', async () => {
  ...
  assert.deepEqual(r, { cardTitle: 'T', cardBody: 'B', followups: ['F1', 'F2', 'F3'], mutter: 'M' });
});
...
test('条子少于 3 条 → 视为没写成,走 fallback', async () => {
  const thin = JSON.stringify({ cardTitle: 'T', cardBody: 'B', followups: ['F1', 'F2'], mutter: 'M' });
  assert.equal(await generateHippoCard(input, { execImpl: async () => ({ stdout: thin }) }), null);
});

test('超长截断:卡面截断,但条子一个字不砍', async () => {
  const long = JSON.stringify({ cardTitle: 't'.repeat(50), cardBody: 'b'.repeat(200), followups: ['q'.repeat(300), 'w', 'e'], mutter: 'm'.repeat(80) });
